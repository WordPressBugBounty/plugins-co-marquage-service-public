		...
		foreach ($bloc_anoter_child as $balise_name => $balise_value) {
			switch ($balise_name) {
				case 'titre':
					...
					echo '&nbsp;' .esc_html($balise_value[0]['text']).'</p>';
					break;
				case 'paragraphe':
					echo '<p class="bloc-paragraphe bloc-anoter-content">'.esc_html($balise_value[0]['text']).'</p>';
					break;
				default:
					if(isset($balise_value[0]['text'])) echo '<p>'.esc_html($balise_value[0]['text']).'</p>';
					break;
			}
		}
	}
